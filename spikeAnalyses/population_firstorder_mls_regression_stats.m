...
scatter(ax,...
   ...
   'SizeData',(E.Explained_Best(E.GroupID=="Intact").*100).^2); 
scatter(ax,...
   E.PostOpDay(~iIntact)+randn(sum(~iIntact),1).*0.10,...
   E.R2_Best(~iIntact),...
   'filled','Marker','x',...
   ...
   'SizeData',(E.Explained_Best(E.GroupID=="Ischemia").*100).^2,...
   'DisplayName','Ischemia'); 
legend(ax,'TextColor','black','FontName','Arial','Location','East');

saveas(fig,fullfile(outPath,'Fig4 - R2 Dynamics Fit by Day and Explained.png'));
savefig(fig,fullfile(outPath,'Fig4 - R2 Dynamics Fit by Day and Explained.fig'));
delete(fig);