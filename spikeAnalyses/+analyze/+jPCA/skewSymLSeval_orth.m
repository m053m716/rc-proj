function [f, g] = skewSymLSeval_orth(m , dX , X)
%% SKEWSYMLSEVAL_ORTH   Evaluate LS function with increased constraint from SKEWSYMLSEVAL
%%%%%%%%%%%%%%%%%%%%%%
...
f = norm( dX - X*analyze.jPCA.reshapeSkew_orth(m) , 'fro')^2;
% f = norm(dX - X*analyze.jPCA.reshapeSkew_orth(m),2); 

%%%%%%%%%%%%%
% Evaluate derivative
%%%%%%%%%%%%%
D = (dX - X*analyze.jPCA.reshapeSkew_orth(m))'*X;
g = 2*analyze.jPCA.reshapeSkew_orth( D - D' ); % return the gradient
