...
if (~params.suppressHistograms) || (params.batchExportFigs)
   fig = figure(...
   ...
   h = bar(ax,binCenters,Y,1,...
      ...
      'EdgeColor','none',...
      'FaceColor','flat'); 
   line(ax,[-pi/2 -pi/2 nan pi/2 pi/2], ...
      [0 yMax*0.75 nan 0 yMax*0.75], ...
      'LineStyle','--',...
      'LineWidth',2,...
      'MarkerFaceColor','b', ...
      'MarkerSize', 8,...
   ...
   if ~isempty(params.Day)
      title(ax,sprintf('%s::%s::Day-%02d (%s)',...
         params.Animal,params.Alignment,params.Day,params.Area),...
         'FontName','Arial',...
         'Color','k','FontWeight','bold');
   end
   ylabel(ax,sprintf('Counts: Phase Angle (Plane-%d)', jPCplane),...
      'FontName','Arial',...
