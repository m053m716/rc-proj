...
Y = nan(numel(S),nBin);
nMax = 0;
theta = cell(1,numel(S));
for iS = 1:numel(S)
   pdThis = [pdMain.(S{iS})];
   theta{iS} = [pdThis.delta];
   Y(iS,:) = histcounts(theta{iS},params.histBins);
...
if (~params.suppressHistograms) || (params.batchExportFigs)
   ...
   axP = polaraxes(fig,...
      ...
      'FontWeight','bold',...
      'ColorOrder',CData);
   for iT = numel(theta):-1:1 % Plot in reverse order to stack most-interesting first
      polarhistogram(axP,theta{iT},params.histBins,...
         'Normalization','pdf',...
         'FaceAlpha',min(0.2+0.2*iT,1));
   end
   if ~isempty(params.Day)
      title(axP,[sprintf('%s::%s::Day-%02d (%s)',...
         params.Animal,params.Alignment,params.Day,params.Area) newline ...
         sprintf('(Plane-%d)', jPCplane)],...
         'FontName','Arial',...
