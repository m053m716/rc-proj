function M = skewSymRegress_orth(dX,X)
%% SKEWSYMREGRESS_ORTH  Find sets of orthogonal subspaces with rotatory structure
%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%        m0 = analyze.jPCA.reshapeSkew_orth(M0k);
m0 = analyze.jPCA.reshapeSkew_orth(M0,false); % override check for skew matrix - MM
% m0 = analyze.jPCA.reshapeSkew_orth(m0);
...
[m, fM, i] = analyze.jPCA.minimize( m0 , 'analyze.jPCA.skewSymLSeval_orth' , 100000, dX , X );
% Aeq = [0ones();
...
% A = [X(1,:); X(3,:); X(
% b;
% Aeq = [1,zeros(1,numel(m0)-2)];
% beq = pi/2;

% m = fmincon(@(m,dX,X)analyze.jPCA.skewSymLSeval_orth(m,dX,X),m0,A,b,Aeq,beq);
...
% return the matrix
M = analyze.jPCA.reshapeSkew_orth(m);
