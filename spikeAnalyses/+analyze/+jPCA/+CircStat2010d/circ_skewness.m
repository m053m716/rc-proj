...
[foo rho2 mu2] = analyze.jPCA.CircStat2010d.circ_moment(alpha,w,2,true,dim);

% compute skewness 
theta2 = repmat(theta, size(alpha)./size(theta));
b = sum(w.*(sin(2*(analyze.jPCA.CircStat2010d.circ_dist(alpha,theta2)))),dim)./sum(w,dim);
b0 = rho2.*sin(analyze.jPCA.CircStat2010d.circ_dist(mu2,2*theta))./(1-R).^(3/2);    % (formula 2.29)
