...
% Mskew_orth = analyze.jPCA.skewSymRegress_orth(dState,preState,optimization_options)'; % and this is the Mskew with sets of orthogonal subspaces -MM
% Mskew_orth = analyze.jPCA.skewSymRegress_orth(dState,preState);
...
%    if pair == 1
%       VconjPair_orth = analyze.jPCA.getRealVs(VconjPair_orth,evConjPair_orth,Ared,nSamples);
%    end
%    
...
if ~exist('params', 'var')
   for ii = 1:n2Plot
      if ~isnan(varCaptEachPlane(ii)) && (vc(ii) > 1e-9)
         analyze.jPCA.plotRosette(Projection, ii, vc(ii),use_orth);
      end
   end
