...
%
%  P = analyze.complete.pca_table(U,K);
%  P = analyze.complete.pca_table(U,K,t_start_stop);
%  [P,C] = ...
%
%  -- Inputs --
%  U : Data table of rates (as uploaded to Tableau)
%     --> Obtained from `U = analyze.successful.get_subset(T);`
%
...
if nargin < 2
   K = defaults.complete_analyses('n_factors');
end
...
% Just for consistency with other packages:
iPositive_to_Negative = cellfun(@(C)mean(C(1:5,1))>0,C.PC_Coeffs);
iTarget = find(iPositive_to_Negative,1,'first');

% Make sure that they are all rotated so components are matched.
Gc = findgroups(C(:,'Alignment'));
for iGroup = 1:max(Gc)
   iC = find(Gc==iGroup);   
   target = C.PC_Coeffs{iTarget};
