...
%       set(ax,'YLim',[mY-rY mY+rY]);
   ...
   case 2
      if isempty(pars.Tag)
         name = sprintf('Mean [95%% CB]: %s & %s',v{1},v{2});
      else
         name = sprintf('Mean [95%% CB]: %s & %s (%s)',...
            v{1},v{2},pars.Tag);
      end
...
%       set(ax,'ZLim',[mZ-rZ mZ+rZ]);
      ...
      for iX = 1:numel(X)
         ...
         iLB = ceil(0.025*nObs);
         iUB = ceil(0.975*nObs);
         thisObs = sort(S.(glme.ResponseName)(iResp),'ascend');
         Zmu_O(iX) = nanmean(thisObs);
         Zmu_P(iX) = nanmean(pred(iResp));
         Zlb95_O(iX) = thisObs(iLB);
         Zub95_O(iX) = thisObs(iUB);
      end
      ...
      surf(X,Y,Zmu_P, ...
         ...
         'Parent',h);
      SZ = min(pars.MinObservationSize+(Zmu_P - Zmu_O).^2,pars.MaxObservationSize);
      scatter3(X(:),Y(:),Zmu_P(:),SZ(:),...
         'Parent',h,...
         'Marker',pars.Marker,...
      ...
      for iX = 1:numel(X)
         ...
         z = max(Zmu_P(iX),Zmu_O(iX));
         % Text displays observed mean and upper/lower 95% CB
         text(ax,X(iX)-0.5,Y(iX),z+dZ,sprintf('%5.2f [%d, %d]',...
            Zmu_O(iX),Zlb95_O(iX),Zub95_O(iX)),...
            'FontName','Arial',...
