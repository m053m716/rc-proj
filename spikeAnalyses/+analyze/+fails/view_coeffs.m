...
%
%  fig = analyze.fails.view_coeffs(C);
%
%  -- Inputs --
%  C : Table returned by `[P,C] = analyze.fails.pca_table(T,K);`
...
if size(C,1) > 1
   ...
   for i = 1:size(C,1)
      if nargout > 0
         fig = [fig; analyze.fails.view_coeffs(C(i,:))]; %#ok<AGROW>
      else
         analyze.fails.view_coeffs(C(i,:));
      end
   end
   return;
end
...
ox = randn(1)*0.05;
oy = randn(1)*0.05;
fig = figure('Name',sprintf('PCA Coeffs (Fails): %s',str),...
   'Units','Normalized',...
   'NumberTitle','off',...
   'Color','w',...
...
if nargout==0
   [path,expr] = defaults.files('fails_fig_dir','fails_view_figs');
   if exist(path,'dir')==0
      mkdir(path);
   end
