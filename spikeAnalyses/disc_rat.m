function [N,D] = disc_rat(X,tol)
%RAT    Rational approximation (TO REPLACE CONFLICT WITH "RAT" CLASS)
...
if ~isreal(X)
    if norm(imag(X),1) <= tol*norm(real(X),1)
        X = real(X);
    elseif nargout > 1
        [NR,DR] = disc_rat(real(X));
        [NI,DI] = disc_rat(imag(X));
        D = lcm(DR,DI);
        N = D./DR.*NR + D./DI.*NI*1i;
        return
    else
        N = char(disc_rat(real(X)),' +i* ...',disc_rat(imag(X)));
        return
    end
end
if nargout > 1
