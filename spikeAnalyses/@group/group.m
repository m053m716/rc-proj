classdef group < matlab.mixin.Copyable
   ...
   methods (Access = public)
      ...
      % Return table of rates (see separate .m file)
...
%          sizeData = size_vals(zi);
      ...
      % Helper function to save RowNames cell array for Rate Table
      function save_table_row_names(RowNames,f_rows)
         %SAVE_TABLE_ROW_NAMES  Static method to save RowNames to file
         %
         %  group.save_table_row_names(RowNames);
         %  group.save_table_row_names(RowNames,f_rows);
         %
         %  -- Inputs --
         %  RowNames : Cell array of Table row names
         %                 (T.Properties.RowNames)
         %  f_rows : Filename of table row names file (for "Rate Table")
