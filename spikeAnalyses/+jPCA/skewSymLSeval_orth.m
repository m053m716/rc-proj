...
f = norm( dX - X*jPCA.reshapeSkew_orth(m) , 'fro')^2;
% f = norm(dX - X*jPCA.reshapeSkew_orth(m),2); 

%%%%%%%%%%%%%
% Evaluate derivative
%%%%%%%%%%%%%
D = (dX - X*jPCA.reshapeSkew_orth(m))'*X;
g = 2*jPCA.reshapeSkew_orth( D - D' ); % return the gradient
