...
R = jPCA.CircStat2010d.circ_r(alpha,w,[],dim);
theta = jPCA.CircStat2010d.circ_mean(alpha,w,dim);
[foo rho2] = jPCA.CircStat2010d.circ_moment(alpha,w,2,true,dim);
[foo foo2 mu2] = jPCA.CircStat2010d.circ_moment(alpha,w,2,false,dim);

% compute skewness 
theta2 = repmat(theta, size(alpha)./size(theta));
k = sum(w.*(cos(2*(jPCA.CircStat2010d.circ_dist(alpha,theta2)))),dim)./sum(w,dim);
k0 = (rho2.*cos(jPCA.CircStat2010d.circ_dist(mu2,2*theta))-R.^4)./(1-R).^2;    % (formula 2.30)
