...
for pindex = 1:length(planes2plot)
   ...
   phaseData = jPCA.getPhase(Projection,plane,Summary.outcomes);
...
%       end
   ...
   if plotPlanEllipse
      jPCA.circle(ellipseRadii,theta_rot,mu);
   end
   ...
   for c = numTrials:-1:1
      ...
      if ismember(c,trials2plot)
         ...
         if isempty(arrowMinVel) || vel > arrowMinVel
            ...
            jPCA.arrowMMC(penultimatePoint, lastPoint, [], aSize, axLim, arrowFaceColor{c}, arrowEdgeColor, arrowAlphaThis);
         else
   ...
   if crossCondMean && length(Summary.crossCondMean) > 1
      ...
      jPCA.arrowMMC(penultimatePoint, lastPoint, [], aSize, axLim, meanColor, meanColor, 1);
   ...
   if useAxes
      ...
      %       haxP = jPCA.AxisMMC(-axisLength, axisLength, axisParams);
      haxP = jPCA.AxisMMC(axLim(1),axLim(2),axisParams);
      ...
      %       vaxP = jPCA.AxisMMC(-axisLength, axisLength, axisParams);
      vaxP = jPCA.AxisMMC(axLim(3),axLim(4),axisParams);
   end
