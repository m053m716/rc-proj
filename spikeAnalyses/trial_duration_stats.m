...
%%
% % Create sub-table for first GLME, with all "considered" data % %
UTrials.Day = UTrials.PostOpDay;
UTrials.Day_Cubed = UTrials.Day.^3;
U_present = UTrials;
U_present(U_present.PelletPresent=="Missing",:) = [];
U_present(isinf(U_present.Duration),:) = [];
...
fig = analyze.behavior.makeDurationDensities(U_present);
...
U_present = utils.addAnimalSpline(U_present,'Duration',3);
glme.trialDuration.duration.mdl = fitglme(U_present,...
...
U_neu = utils.addAnimalSpline(U_neu,'Duration',3);
...
glme.trialDuration.reach.mdl = fitglme(U_neu,...
   "Reach_Epoch_Duration~1+GroupID*Day+(1+Day|AnimalID)",...
   "FitMethod","REMPL",...
   "Distribution",'normal',...
   "DummyVarCoding",'effects',...
   "Link",'log');
...
glme.trialDuration.retract.mdl = fitglme(U_neu,...
   "Retract_Epoch_Duration~1+GroupID*Day+(1+Day|AnimalID)",...
   "FitMethod","REMPL",...
   "Distribution",'normal',...
   "DummyVarCoding",'effects',...
   "Link",'log');
...
glme.trialDuration.proportion.mdl = fitglme(U_neu,...
   "Reach_Epoch_Proportion~1+GroupID*Day+(1+Day|AnimalID)",...
   "FitMethod","REMPL",...
   "Distribution",'normal',...
   "DummyVarCoding",'effects',...
   "Link",'log');
glme.trialDuration.proportion.id = 9;
disp('Recovered GLME for Models 5-9');
...
[fig,mdl_duration] = analyze.behavior.per_animal_mean_trends(...
   U_present,...    % First required argument is always the data table
   'Duration',... % Second required argument is the response variable name
   'Title','Total Reach Duration',...
   'DoExclusions',false,...
   'ModelFormula','%s~1+Day', ... % Optional 'ModelFormula' is per-animal trend to fit
   'LegendStyle','animals',...
...
fig = analyze.behavior.per_animal_mean_trends(...
   U_neu,...    % First required argument is always the data table
   'Duration',... % Second required argument is the response variable name
...
fig = analyze.behavior.per_animal_mean_trends(...
   ...
   'DoExclusions',false,...
   'ModelFormula','%s~1+Day+Duration', ... % Optional 'ModelFormula' is per-animal trend to fit
   'LegendStyle','animals',...
   'LegendLocation','eastoutside' ...
   );
